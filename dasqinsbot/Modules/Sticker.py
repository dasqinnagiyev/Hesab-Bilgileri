...
# MODED BY @dasqinnagiyev

@dasqinsbot.on_message(filters.private & ~filters.forwarded & ~filters.command(["start", "yardim", "haqqinda", "info", "information", "json", "source", "id"]))
async def stickers(dasqin, msg):
    ...
    if update_channel:
        try:
            ...
            return  
@dasqinsbot.on_message(filters.command(["stickerid"]))
async def stickerid(bot, message):   
    if message.reply_to_message.sticker:
       await message.reply(f"**Sənin sticker id**  \n `{message.reply_to_message.sticker.file_id}` \n \n ** Unikal ID ** \n\n`{message.reply_to_message.sticker.file_unique_id}`", quote=True)
    else: 
       await message.reply("Hmm bu sticker deyil...!!!")
